...
const extractArray = (payload: any): Discussion[] => {
  if (!payload) return [];
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchDiscussions.fulfilled, (state, action) => {
        state.isLoading = false;
        state.discussions = extractArray(action.payload);
      })
      ...
      .addCase(fetchPopularDiscussions.fulfilled, (state, action) => {
        state.isLoading = false;
        state.popularDiscussions = extractArray(action.payload);
      })
      ...
      .addCase(fetchBookmarkedDiscussions.fulfilled, (state, action) => {
        state.isLoading = false;
        state.bookmarkedDiscussions = extractArray(action.payload);
      })
      ...
      .addCase(addReply.fulfilled, (state, action) => {
        ...
        // Update in bookmarked discussions
        const bookmarkedIndex = state.bookmarkedDiscussions.findIndex(d => d.id === parentId);
        if (bookmarkedIndex !== -1) {
          if (!state.bookmarkedDiscussions[bookmarkedIndex].replies) {
            state.bookmarkedDiscussions[bookmarkedIndex].replies = [];
          }
          state.bookmarkedDiscussions[bookmarkedIndex].replies.push(reply);
          state.bookmarkedDiscussions[bookmarkedIndex].reply_count += 1;
        }
      });
  },
});
